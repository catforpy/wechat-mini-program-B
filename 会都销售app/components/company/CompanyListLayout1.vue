<template>
  <view class="company-list-layout1">
    <!-- 按公司类型分组 -->
        ...
        >
          ...
          <view v-if="company.expanded" class="mini-programs-list">
            ...
            <view
              ...
              @tap.stop="openAddProgramModal(company)"
            >
    ...
    <AddProgramModal
      ...
      :authenticated-companies="filteredCompanies"
...
const emit = defineEmits<{
  ...
  'addProgram': [useNewCompany: boolean, company: AuthenticatedCompany | null, targetCompany: Company]
}>()
...
const filteredCompanies = computed(() => {
  if (!currentCompany.value) return props.authenticatedCompanies
  return props.authenticatedCompanies.filter(
    company => company.type === currentCompany.value?.type
  )
})
...
// 打开添加小程序弹窗
const openAddProgramModal = (company: Company) => {
...
const handleModalConfirm = (useNewCompany: boolean, companyIndex: number) => {
  if (useNewCompany) {
    // 使用新注册企业
    emit('addProgram', true, null, currentCompany.value!)
  } else {
    // 使用已认证企业
    const company = filteredCompanies.value[companyIndex]
    emit('addProgram', false, company, currentCompany.value!)
  }
}
