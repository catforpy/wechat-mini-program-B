  ...
  <view class="business-info-page">
    ...
    <view class="nav-bar" :style="{ paddingTop: statusBarHeight + 'px' }">
      <view class="nav-content">
        ...
        <text class="nav-title">经营信息</text>
    ...
    <scroll-view class="content-scroll" scroll-y>
      <view class="form-container">
        ...
        <view class="form-item avatar-item">
          <text class="form-label">经营者头像</text>
        ...
        <view class="form-item">
          <text class="form-label required">经营者姓名</text>
          <input
            class="form-input"
        ...
        <!-- 经营地址 -->
        <view class="form-item">
          <text class="form-label required">经营地址</text>
          <input
            class="form-input"
        ...
        <view class="tips-section">
          ...
          <text class="tips-text">经营信息将用于名片展示和商务联系，请确保信息真实准确</text>
        </view>
      </view>
    </scroll-view>
  </view>
    ...
    <view class="consent-modal-content" @tap.stop>
      ...
      <view class="consent-body">
        <text class="consent-text">
          为了完善您的个体工商户名片信息，我们需要收集经营基本信息：
        </text>

        <view class="consent-list">
          <text class="consent-item">• 经营者姓名、店铺名称：用于身份识别</text>
          <text class="consent-item">• 税号：用于税务登记识别</text>
          <text class="consent-item">• 地址、联系方式：用于商务联系</text>
          <text class="consent-item">• 经营者头像：用于形象展示</text>
        </view>

        <text class="consent-notice">
          我们承诺：您的经营信息将严格保密，仅用于名片展示和商务联系方式展示，不会泄露给第三方。
        </text>
      </view>
...
const checkFirstTime = () => {
  const hasAgreed = uni.getStorageSync('business_info_consented')
  if (!hasAgreed) {
...
// 同意授权
const handleAgree = () => {
  uni.setStorageSync('business_info_consented', true)
...
const handleSave = () => {
  ...
  // TODO: 保存到本地存储或服务器
  uni.setStorageSync('business_info_data', JSON.stringify(formData.value))
...
onMounted(() => {
  ...
  // 加载已保存的数据
  const savedData = uni.getStorageSync('business_info_data')
  if (savedData) {
    try {
      formData.value = JSON.parse(savedData)
    } catch (e) {
      console.error('读取经营信息失败:', e)
    }
  }
