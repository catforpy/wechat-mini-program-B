<template>
  <view class="leasing-application-page">
    <!-- 顶部导航栏 -->
    <view class="nav-bar" :style="{ paddingTop: statusBarHeight + 'px' }">
      <view class="nav-content">
        ...
        <text class="nav-title">申请租赁</text>
    ...
    <scroll-view class="main-content" scroll-y>
      ...
      <view class="form-section">
        <text class="section-title">申请租赁的小程序</text>
        <view v-if="targetProgram" class="program-card">
          <view class="program-left">
            ...
            <view class="program-info">
              ...
              <text v-if="targetProgram.price" class="program-price">{{ targetProgram.price }}</text>
            </view>
          </view>
        </view>
      </view>
      ...
      <view class="form-section">
        ...
        <view v-if="!isRealNameVerified" class="unverified-notice">
          ...
          <view class="notice-content">
            ...
            <text class="notice-desc">申请租赁需要先完成实名认证，实名后可以使用个人身份或公司主体进行申请</text>
          </view>
      ...
      <!-- 租赁期限 -->
      <view class="form-section">
        <text class="section-title required">租赁期限</text>
        <view class="period-options">
          <view
            v-for="period in leasingPeriods"
            :key="period.value"
            :class="['period-option', { selected: formData.leasingPeriod === period.value }]"
            @tap="selectLeasingPeriod(period.value)"
          >
            <text class="period-name">{{ period.label }}</text>
            <text class="period-desc">{{ period.description }}</text>
          </view>
        </view>
      </view>
      ...
      <!-- 租金预算 -->
      <view class="form-section">
        <text class="section-title required">月租金预算</text>
        <view class="price-input-wrapper">
          ...
          <input
            ...
            type="digit"
            class="price-input"
            placeholder="请输入月租金预算"
            @input="onFieldChange"
          />
          <text class="price-suffix">元/月</text>
        </view>
        <view class="budget-hints">
          <text class="hint-text">参考价格：{{ getReferencePrice() }}</text>
        </view>
      </view>

      <!-- 租赁开始时间 -->
      <view class="form-section">
        <text class="section-title required">期望开始时间</text>
        <view class="date-picker" @tap="showStartDatePicker">
          <text v-if="formData.startDate" class="date-text">{{ formData.startDate }}</text>
          <text v-else class="date-placeholder">请选择租赁开始时间</text>
      ...
      <view class="notice-section">
        ...
        <text class="notice-item">• 小程序主体公司审批通过后，即可开始租赁</text>
        <text class="notice-item">• 租赁期间，小程序的所有权仍归原主体所有</text>
...
// 租赁期限选项
const leasingPeriods = [
  { value: '3', label: '3个月', description: '短期试用' },
  { value: '6', label: '6个月', description: '短期合作' },
  { value: '12', label: '12个月', description: '标准期限' },
  { value: '24', label: '24个月', description: '长期租赁' }
]
...
interface LeasingApplication {
  ...
  leasingPeriod: string                  // 租赁期限（月）
  monthlyBudget: string                  // 月租金预算
  startDate: string                      // 开始时间
  purpose: string                        // 申请目的
  additionalInfo: string                 // 补充说明
}

const formData = ref<LeasingApplication>({
...
// 选择租赁期限
const selectLeasingPeriod = (period: string) => {
  formData.value.leasingPeriod = period
  onFieldChange()
}

// 获取参考价格
const getReferencePrice = () => {
  const period = parseInt(formData.value.leasingPeriod)
  if (period >= 24) {
    return '¥299/月（长期优惠）'
  } else if (period >= 12) {
    return '¥399/月'
    ...
    success: (res) => {
      if (res.confirm && res.content) {
        ...
        if (dateRegex.test(res.content)) {
          formData.value.startDate = res.content
          onFieldChange()
        } else {
          uni.showToast({
            title: '日期格式不正确',
...
const validateForm = (): { valid: boolean; message?: string } => {
  ...
  if (!formData.value.monthlyBudget || parseFloat(formData.value.monthlyBudget) <= 0) {
    return { valid: false, message: '请输入月租金预算' }
  }

  if (!formData.value.startDate) {
    return { valid: false, message: '请选择租赁开始时间' }
  }
...
const submitApplication = () => {
  ...
  const periodText = leasingPeriods.find(p => p.value === formData.value.leasingPeriod)?.label

  uni.showModal({
    title: '确认提交',
    content: `确认向「${targetProgram.value.name}」提交租赁申请吗？\n\n申请主体：${applicantName}\n租赁期限：${periodText}\n月租金：¥${formData.value.monthlyBudget}`,
    confirmText: '确认提交',
    cancelText: '取消',
    success: (res) => {
      if (res.confirm) {
        // 保存申请到本地存储
        const applications = uni.getStorageSync('leasing_applications') || []
        ...
        const newApplication = {
          ...
          targetProgram: {
            ...
            price: targetProgram.value.price
          },
          applicantInfo: applicantInfo,
          leasingPeriod: formData.value.leasingPeriod,
        ...
        uni.setStorageSync('leasing_applications', applications)
